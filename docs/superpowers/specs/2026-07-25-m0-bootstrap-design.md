...
client that is closed on shutdown. The client registers an `error` listener: ioredis
emits `error` on every reconnection attempt, and an `EventEmitter` with no listener
for that event throws, which would crash the process precisely in the scenario the
...
`Kysely<any>` is what Kysely's own documentation prescribes for migration files —
a migration runs against whatever schema exists at that point in history, not
against the current `DB` interface.
...
- Unit tests for the two pieces of M0 that hold actual logic: the environment schema
  (rejects a missing `DATABASE_URL`, applies the `PORT` default, coerces it to a
  number) and the probe timeout helper (resolves the underlying promise, rejects once
  the deadline passes, clears its timer).

Nothing else gets a unit test. Asserting that a configuration getter returns its
value tests the framework rather than the code.

### npm scripts

`start:dev` · `build` · `lint` · `format` · `test` · `test:e2e` · `migrate` ·
`migrate:down`

`migrate` runs under `node --env-file-if-exists=.env --import tsx`, so it reads
`.env` locally and falls back to the process environment in CI, where no such file
exists.
